...
namespace FronkonGames.LUTs.Synthwave
{
  ...
  public sealed partial class Synthwave
  {
    ...
    private static readonly Synthwave[] NoEffects = new Synthwave[0];
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    public static Synthwave Instance
    {
      get
      {
        ...
        if (pipelineAsset != null)
        {
          ...
          {
            if (scriptableRendererData.rendererFeatures[i] is Synthwave)
              return scriptableRendererData.rendererFeatures[i] as Synthwave;
          }
        }
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    public static Synthwave[] Instances
    {
      get
      {
          ...
          for (int i = 0; i < rendererDataList.Length; ++i)
          {
            if (rendererDataList[i] != null && rendererDataList[i].rendererFeatures.Count > 0)
              foreach (var feature in rendererDataList[i].rendererFeatures)
                if (feature is Synthwave)
                  effects.Add(feature as Synthwave);
          }
