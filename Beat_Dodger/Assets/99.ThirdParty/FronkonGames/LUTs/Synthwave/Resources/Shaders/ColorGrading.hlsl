...
float3 ColorGrading(float3 pixel)
{
  ...
  // Vibrance.
  float saturationAmount = max3(pixel) - min3(pixel);
  float vibranceAmount = 1.0 + _Vibrance * (1.0 - saturationAmount);
  pixel = (pixel - luminance) * max(vibranceAmount, 0.0) + luminance;
